...
extends WikiPlugin
{
    ...
    function run($dbi, $argstr, $request) {
		...
		if(count($people) < 1){
			$html = HTML("Nobody has used $PAGE_TEXT_LABEL on their homepage");
			return $html;	
		}
		...
		$td = HTML::td(array('bgcolor' => '#FFFFFF'));
		$td->pushContent("Total Units");
		$tr->pushContent($td);	
		$td = HTML::td(array('bgcolor' => '#FFFFFF'));
		$td->pushContent("Total Voters");
		$tr->pushContent($td);	
		...
		$td->pushContent(HTML::strong("Total Budget"));
		$tr->pushContent($td);
		$cans_total = 0;
		$total_voters = 0;
