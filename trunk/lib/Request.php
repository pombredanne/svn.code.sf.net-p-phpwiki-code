...
class Request
{
    ...
    public function get($key)
    {
        ...
        } // cgi or other servers than Apache
        else {
            trigger_error(
                "Serious php configuration error!"
    ...
    public function getURLtoSelf($args = array(), $exclude = array())
    {
        $get_args = $this->args;
        if ($args) {
    ...
    public function finish()
    {
        ...
        if (!empty($this->_accesslog)) {
            ...
            global $RUNTIMER;
            if ($RUNTIMER) {
                $this->_accesslog->setDuration($RUNTIMER->getTime());
            }
    ...
    public function setCookieVar($key, $val, $lifetime_in_days = false, $path = false)
    {
        $this->cookies->set($key, $val, $lifetime_in_days, $path);
    }

    public function deleteCookieVar($key)
    {
...
class Request_CookieVars
{
    ...
    public function set($key, $val, $persist_days = false, $path = false)
    {
...
class Request_AccessLog
{
     ...
     * @param bool $do_sql
     */
    public function __construct($logfile, $do_sql = false)
    {
        $this->logfile = $logfile;
        if ($logfile and !is_writeable($logfile)) {
            trigger_error(sprintf(_("%s is not writable."), _("The PhpWiki access log file"))
                    . "\n"
                    . sprintf(
     ...
     */
    public function get_referer($limit = 15, $external_only = false)
    {
    ...
    {
        if ($this->logfile) {
            $this->logfile = ACCESS_LOG;
        } // support Request_AccessLog::read
    ...
    public function _read_sql_query($where = '')
    {
        global $request;
        $dbh =& $request->_dbi;
    ...
    public function read_sql($where = '')
    {
...
class Request_AccessLogEntry
{
     ...
     *
     */
    public function __construct(&$accesslog)
    {
    ...
    public function write_sql()
    {
        ...
        if ($dbh and $dbh->isOpen() and $this->_accesslog->logtable) {
            ...
            if ($request->get('REQUEST_METHOD') == "POST") {
                ...
                // garble passwords
                if (!empty($args['auth']['passwd'])) {
                    $args['auth']['passwd'] = '<not displayed>';
                }
                if (!empty($args['dbadmin']['passwd'])) {
                    $args['dbadmin']['passwd'] = '<not displayed>';
                }
                if (!empty($args['pref']['passwd'])) {
                    $args['pref']['passwd'] = '<not displayed>';
                }
                if (!empty($args['pref']['passwd2'])) {
                    $args['pref']['passwd2'] = '<not displayed>';
                }
...
class HTTP_ETag
{
     ...
     *
     */
    public static function parse($strval)
    {
...
class HTTP_ValidatorSet
{
    ...
    public function checkConditionalRequest(&$request)
    {
        ...
        if ($result == _HTTP_VAL_PASS || $result == _HTTP_VAL_MODIFIED) {
            return false;
        } // "please proceed with normal processing"
