...
function LinkImage($url, $alt = "")
{
    ...
    foreach ($arr as $attr => $value) {
        ...
        elseif ($attr == "size") {
            ...
            return HTML::span(
                array('class' => 'error'),
                sprintf(
                    _("Invalid attribute “%s=%s” for image “%s”"),
                    $attr,
                    $value,
                    $url
                )
    ...
    if ($url != $ori_url and empty($arr) and !preg_match("/^http/", $url)) {
        ...
        if (file_exists($file)) {
            ...
            trigger_error(
                sprintf(
                    _("Invalid image link fixed “%s” => “%s”. Spaces must be quoted with %%20."),
                    $url,
                    $ori_url
                ),
                E_USER_WARNING
...
function ImgObject($img, $url)
{
    ...
    $object = HTML::object(array_merge(
        $img->_attr,
        array('type' => $type)
    ), //'src' => $url
        $img->_content);
...
class Alert
{
     ...
     */
    public function __construct($head, $body, $buttons = array())
    {
        if (is_array($body)) {
            $html = HTML::ol();
            foreach ($body as $li) {
...
function extractSection($section, $content, $page, $quiet = false, $sectionhead = false)
{
    ...
    if (preg_match(
        "/ ^(!{1,}|={2,})\\s*$qsection\s*=*" // section header
