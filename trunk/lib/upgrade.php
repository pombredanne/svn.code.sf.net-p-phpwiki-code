...
class Upgrade
{
    ...
    public function CheckPgsrcUpdate()
    {
        ...
        if ($pgsrc->getFiles()) {
            echo "<h2>", sprintf(
                _("Check for necessary theme %s updates"),
                "pgsrc"
            ), "</h2>\n";
            foreach ($pgsrc->getFiles() as $filename) {
                if (substr($filename, -1, 1) == '~') {
    ...
    {
        echo "<h2>", sprintf(
            _("Check for necessary %s updates"),
            "config.ini"
        ), "</h2>\n";
...
class UpgradeEntry
{
    ...
    public $check_cb;
    public $reason;
    /* array */ public $applicable_args;
    /* object */ public $parent;
    /* array */ private $check_args;
    /* string */ private $notice;
    /* string */ private $_db_key;
    private $upgrade;
    ...
    public function __construct(&$parent, $params)
    {
        ...
        foreach (array('key' => 'required',
            ...
            if (!isset($params[$k])) { // default
                if ($v == 'required') {
                    trigger_error("Required arg $k missing", E_USER_ERROR);
                } else {
                    $this->{$k} = $v;
                }
            } else {
