 ...
 * Once the icons for the buttons have been finalized, the alpha channel
 * will be added to eliminate the white parts.
 *
 * The button toolbars use tables for positioning. Yuck. (It will do for now).
 *
 * RSS has it's own Mac OS X style button but it's not actually used yet.
 *
 * In Mozilla, there is a grey line under each of the images. I haven't 
 * been able to determine the cause of this yet.
 *
 * Only English buttons are available. Full localization is still a ways
 * off. Work is in progress to modularize & tokenize the normal templates
 * and (somehow) some cool toolbar functions are going to result from it.
 ...
 * look and feel like a Mac interface: with subtle effects and a fine
 * color scheme. Since I'm sick of the stripes Mr. Jobs thoughtfully plastered
 * all over my screen, I've chosen a brushed paper (or stucco?) texture effect
 * very close to white. If your monitor isn't calibrated well you might not
 * even see it.
 *
 * I probably won't be submitting anything else for this theme for a bit.
 * Not until the default toolbar stuff and templates are further along anyway.
 * 
 * Send me some feedback, do you like the icons used in the buttons? Got
 * any ideas for code to pick out the localized buttons from the right
 * directory? Automatic button generation for localized buttons isn't going
 * to happen for this theme--there is a gradient across the glass surface of
 * the button that only Mac OS X Aqua can generate. Chopping a button up and
 * stamping it with localized words means a lot of tweaking to the blank
 * button pieces to get the seams invisible. So it will be a nicer effect
 * to produce them by hand.
 *
 * The current link icons I want to move into this theme, and come up with
 * some new linkicons for the default look. (Comments, feedback?)
 *
 * */
